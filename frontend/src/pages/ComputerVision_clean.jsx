...
const ComputerVision = () => {
  ...
  const processImageWithBackendAI = async (imageFile) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/computer-vision/detect', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        return {
          ...
          total_products_detected: result.detected_objects?.length || 0,
          processing_time: result.processing_time || 0,
          detected_products: result.detected_objects?.map((obj, index) => ({
            product_id: index + 1,
            detected_count: 1,
            confidence_score: obj.confidence,
            message: `Detected ${obj.name}`,
            category: obj.category || 'General Merchandise',
            bbox: obj.bbox
          })) || [],
          api_source: result.ai_service || "Backend AI Service",
          real_ai: true,
          success: true
        };
      } else {
